...
@require_role(["admin"])
async def list_users_route(db: AsyncSession = Depends(get_db), user = Depends(get_current_user)):
...
@require_role(["admin"])
async def get_user_route(user_id: int, db: AsyncSession = Depends(get_db), user = Depends(get_current_user)):
...
@require_role(["admin"])
async def update_user_route(user_id: int, user_data: UserUpdate, db: AsyncSession = Depends(get_db), user = Depends(get_current_user)):
...
@require_role(["admin"])
async def delete_user_route(user_id: int, db: AsyncSession = Depends(get_db), user = Depends(get_current_user)):
