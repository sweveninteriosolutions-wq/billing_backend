...
router = APIRouter(prefix="/inventory/alerts", tags=["Inventory Stock Alerts"])
...
async def stock_alerts(db: AsyncSession = Depends(get_db), _user=Depends(get_current_user)):
    """
    Fetch all products that are below minimum stock thresholds.
    Only active products are considered.
    Logs the check activity for the current user.
    """
    return await get_stock_alerts(db, current_user=_user)
